...
def run_mrp(export_file, stock_file, bom_file, receipt_file):
    ...
    def get_sfrac(rows, comp_col, gross_col):
        # This function is intended to calculate the 'shortage fraction'
        # It iterates through each component and its demands over time.
        # 'avail' tracks the available stock for that component.
        # 'sfrac' stores the fraction of demand *not* covered by available stock.
        # The error occurs here because 'rows' might be empty or not have the expected columns.
    ...
    def apply_sfrac(df2, gross_col, ph_col, sfrac_dict, comp_col):
        # This function applies the calculated shortage fraction.
        # If a component is a phantom, its gross requirement is not reduced by sfrac.
        # Otherwise, the gross requirement is multiplied by the sfrac, effectively
        # reducing the demand being passed up if there's a calculated shortage.
        return df2.apply(
    ...
    # FIX: Ensure 'l2_norm' is not empty
    if not l2_norm.empty:
    ...
    # FIX: Ensure 'l3_norm' is not empty
    if not l3_norm.empty:
